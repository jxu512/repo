...
async def call_mcp_tool(a: int, b: int) -> str:
    ...
    async with Client("http://localhost:8182/mcp") as client:
        print(f"Client connected: {client.is_connected()}")
        tools = await client.list_tools()
        print("Available tools:", tools)
        response = await client.call_tool("subtract_numbers_tool", {"a": 6, "b": 6})
        response = await client.call_tool("subtract_numbers_tool", request.model_dump())
